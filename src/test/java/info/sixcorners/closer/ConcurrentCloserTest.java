...
class ConcurrentCloserTest {
  @Mock AutoCloseable closeable;
  @Mock AutoCloseable closeableB;
  ConcurrentCloser closer = new ConcurrentCloser();
  ...
  @Test
  void closeForGood() throws Exception {
    closer.onClose(closeable);
    closer.closeForGood();
    verify(closeable).close();
    assertThrows(IllegalStateException.class, () -> closer.onClose(closeable));
  }
}
